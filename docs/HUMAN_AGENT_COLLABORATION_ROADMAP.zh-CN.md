...
- Task 已经有 `nextAction`、`definitionOfDone`、优先级、风险等级、
  agent tags、owner agent、lease、heartbeat、contribution、artifact 和 review。
...
| 没有 DailyPlan 快照 | planner 是实时生成的，但当天主线、第一步动作和通知结果没有固化为对象。 |
| 没有明确 executionMode | `riskLevel` 表示危险程度，但还没有 `manual`、`agent_suggested`、`agent_allowed`、`approval_required` 这种执行授权。 |
| 没有 Agent 能力注册表 | 任务有 tags，但系统不知道哪个 Agent 能浏览、写代码、调用 API、写提醒或操作文件。 |
...
| Wiki 学习闭环不完整 | 任务结果能链接 Wiki 证据，但通过复核后的结论不会自动变成 Wiki/Workflow 更新建议。 |
...
任务分发不应该只看 tag，还要看执行模式、风险等级和 Agent 能力。
...
1. 收紧 task claim / heartbeat / contribution 的归属规则。
2. 给 Task 增加 `executionMode`，阻止高风险任务被自动认领。
3. 增加 `AgentProfile`，让 Agent inbox 按能力过滤。
4. 增加 `DailyPlan`，保存 planner 输出和通知结果。
5. 增加 `TaskRun`，记录任务执行尝试。
6. 增加结构化 `AgentActionLog`。
7. 增加 intake decision schema 和质量评分。
8. 增加 workflow/skill notes，并接入 agent context。
