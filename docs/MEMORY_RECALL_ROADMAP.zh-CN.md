...
- **已完成**：
  ...
  - 部署：OS + Wiki 镜像重建到线上；compose.production.yml 补 token env
  - 评测脚本：支持 PERSONAL_OS_READ_TOKEN header
  - 单测：agent-context + query-intent + wiki-client + wiki-ingest **51/51 通过**
- **线上 B0 对比**（28 题）：

  | 指标 | B0 基线 | P2 后 | 变化 |
  | --- | --- | --- | --- |
  | P50 延迟 | 52ms | 169ms | FTS chunk + 元数据水合 |
  | P95 延迟 | 101ms | 467ms | 同上；超 300ms 目标 |
  | 平均候选 | 4.5 | 5.0 | 略增 |
  | title 命中率 | 0.041 | 0.087 | P1 意图加权生效 |
  | 平均 memoryItems | 6.96 | 6.54 | 持平 |
  | evidence_tokens (wiki.candidates) | 486 | 0 | 设计变更：excerpt 移至 evidence.cards |

  注：evidence_tokens=0 是评测脚本测量 wiki.candidates[].excerpt（P2 后改为元数据视图，无 excerpt）。实际证据在 evidence.cards[].text 和 memoryItems[].summary，评测脚本需更新以测量新字段。

  待解问题：
  - P95 延迟超标（467ms vs 300ms 目标）- 需优化 FTS chunk 搜索和元数据水合
  - 噪声查询返回 1 候选（B0 为 0）- FTS 模糊匹配回退
  - 评测脚本需增加 evidence.cards token 统计

- **未竟**：P3 生命周期/鉴权；评测脚本更新；延迟优化
- **下次第一刀**：更新评测脚本测量 evidence.cards；排查 P95 延迟；P3
