...
vi.mock("@/lib/wiki-client", () => ({
  ...
  readWikiNote: wikiMocks.readWikiNote,
  searchWikiNotes: wikiMocks.searchWikiNotes,
  searchWikiChunks: wikiMocks.searchWikiChunks,
...
const mockedSearchWikiChunks = wikiMocks.searchWikiChunks;
const mockedExpandWikiChunk = wikiMocks.expandWikiChunk;
const mockedReadWikiNote = wikiMocks.readWikiNote;

describe("agent context harness", () => {
  beforeEach(() => {
    mockedSearchWikiNotes.mockReset();
    mockedExpandWikiChunk.mockReset();
    mockedSearchWikiChunks.mockReset();
    mockedSearchWikiChunks.mockImplementation(async (query: string, limit: number) => {
      const notes = await mockedSearchWikiNotes(query, limit);
      return {
        status: "ok",
        ...
        results: notes.map((note: Record<string, unknown>) => ({
          ...
          source_type: note.source_type ?? "agent-output",
  ...
  it("resolves required Wiki chunks exactly and keeps them ahead of fuzzy recall", async () => {
    ...
    expect(mockedReadWikiNote).toHaveBeenCalledWith(
      "vault/memory-eval.md",
  ...
  it("falls back to note search while the structured index is rebuilding", async () => {
    mockedSearchWikiChunks.mockRejectedValue(new Error("rebuilding-index"));
    mockedSearchWikiNotes.mockResolvedValue([
      {
        title: "Personal OS 召回",
        path: "vault/20_notes/recall.md",
        status: "active",
        source_type: "manual",
        excerpt: "Personal OS 召回仍可使用旧索引。",
      },
    ]);

    const result = await searchWikiContext(["Personal OS 召回"], 8);

    expect(result.status).toBe("ok");
    expect(result.candidates[0].metadata).toMatchObject({
      retrieval: "wiki-notes-fallback",
    });
  });

  it("keeps full chunk text out of the Agent context candidate", async () => {
      ...
      results: [
        {
          ...
          path: "vault/20_notes/targeted.md",
          status: "active",
          source_type: "manual",
          text: "This is the complete indexed chunk and must not leak into context.",
          snippet: "Targeted evidence only.",
          chunk_id: "chunk:1",
          section_id: "section:1",
          start_char: 120,
          end_char: 240,
          estimated_tokens: 6,
          available_estimated_tokens: 60,
        },
      ],
    });

    const result = await searchWikiContext(["Targeted evidence"], 8);

    expect(result.candidates[0]).not.toHaveProperty("text");
    expect(result.candidates[0].excerpt).toBe("Targeted evidence only.");
    expect(result.candidates[0].metadata).toMatchObject({
      retrieval: "fts5-bm25-structured-v2",
      chunk_id: "chunk:1",
      section_id: "section:1",
      start_char: 120,
      end_char: 240,
      snippet_estimated_tokens: 6,
      chunk_estimated_tokens: 60,
    });
  });

  it("preserves strict Wiki heading priority over a stronger title-substring rescore", async () => {
    mockedSearchWikiChunks.mockResolvedValue({
      status: "ok",
      retrieval: "fts5-bm25-structured-v2",
      results: [
        {
          ...
          path: "vault/20_notes/title-substring.md",
          status: "active",
          source_type: "manual",
          snippet: "small",
        ...
        {
          title: "Unrelated long document",
          path: "vault/20_notes/exact-heading.md",
          status: "active",
          source_type: "manual",
          snippet: "OAuthToken precise section evidence",
          chunk_id: "chunk:heading",
          match_type: "exact-heading",
          heading_path: ["OAuthToken"],
        },
      ],
    });

    const result = await searchWikiContext(["OAuthToken"], 2);

    expect(result.candidates.map((candidate) => candidate.path)).toEqual([
      "vault/20_notes/exact-heading.md",
      "vault/20_notes/title-substring.md",
    ]);
    expect(result.candidates[0].metadata).toMatchObject({
      match_type: "exact-heading",
      heading_path: ["OAuthToken"],
    });
    expect(result.candidates[0].metadata).not.toHaveProperty(
      "structuredPriority",
    );

    const context = await getQueryAgentContext("OAuthToken", undefined, {
      budgetTokens: 1800,
      topK: 8,
    });
    const wikiItems = context.memoryItems.filter((item) => item.type === "wiki");
    expect(wikiItems.map((item) => item.metadata.path)).toEqual([
      "vault/20_notes/exact-heading.md",
      "vault/20_notes/title-substring.md",
    ]);
    const serializedMemory = JSON.stringify(context.memoryItems);
    expect(serializedMemory).not.toContain("retrievalPriority");
    expect(serializedMemory).not.toContain("retrievalRank");
  });

  it("expands only explicit context requests with a bounded server budget", async () => {
    mockedSearchWikiChunks.mockResolvedValue({
      status: "ok",
      retrieval: "fts5-bm25-structured-v2",
      results: [
        {
          ...
          path: "vault/20_notes/deployment.md",
          status: "active",
          source_type: "manual",
          snippet: "短片段",
    ...
    mockedExpandWikiChunk.mockResolvedValue({
      status: "ok",
      level: "neighbor",
      content: "包含前后两段的证据",
      range_basis: "indexed-snapshot",
      estimated_tokens: 20,
      available_estimated_tokens: 20,
      truncated: false,
    });

    const result = await searchWikiContext(["请展开 Deployment evidence 的前后文"], 8);

    expect(mockedExpandWikiChunk).toHaveBeenCalledWith(
      "chunk:deployment",
      "neighbor",
      expect.objectContaining({
        maxTokens: 700,
        query: "请展开 Deployment evidence 的前后文",
      }),
    );
    expect(result.candidates[0].excerpt).toBe("包含前后两段的证据");
    expect(result.candidates[0].metadata).toMatchObject({
      expansion_level: "neighbor",
      expansion_range_basis: "indexed-snapshot",
      expansion_truncated: false,
    });
  });

  it("does not expand when context is only the retrieval topic", async () => {
    mockedSearchWikiChunks.mockResolvedValue({
      status: "ok",
      retrieval: "fts5-bm25-structured-v2",
      results: [
        {
          title: "Personal OS context recall",
          path: "vault/20_notes/context-recall.md",
          status: "active",
          source_type: "manual",
          snippet: "召回主题的短片段",
    ...
    const result = await searchWikiContext(["Personal OS Agent 上下文召回"], 8);

    expect(result.candidates[0].excerpt).toBe("召回主题的短片段");
    expect(mockedExpandWikiChunk).not.toHaveBeenCalled();
  });

  it("does not treat bare handbook labels as an instruction to expand", async () => {
    mockedSearchWikiChunks.mockResolvedValue({
      status: "ok",
      retrieval: "fts5-bm25-structured-v2",
      results: [
        {
          title: "Personal OS 操作资料",
          path: "vault/20_notes/operations.md",
          status: "active",
          source_type: "manual",
          snippet: "短片段",
    ...
    for (const query of ["Personal OS SOP", "Personal OS runbook", "Personal OS 操作手册"]) {
      await searchWikiContext([query], 8);
    }

    expect(mockedExpandWikiChunk).not.toHaveBeenCalled();
  });

  it("returns path-deduped wiki evidence cards with hard top and expand handles", async () => {
    mockedSearchWikiChunks.mockResolvedValue({
      status: "ok",
      retrieval: "fts5-bm25-structured-v2",
      results: [
        {
          title: "Deploy SOP A",
          path: "vault/20_notes/deploy-a.md",
          status: "active",
          source_type: "manual",
          snippet: "第一步安装 docker compose，第二步配置环境变量并启动服务。",
          chunk_id: "chunk:a1",
          heading_path: ["部署", "安装"],
          start_char: 10,
          end_char: 80,
          expand: {
            neighbor: "/api/search/chunks/expand?chunk_id=a1&level=neighbor",
            section: "/api/search/chunks/expand?chunk_id=a1&level=section",
            document: "/api/search/chunks/expand?chunk_id=a1&level=document",
          },
          score: 12,
        },
        {
          title: "Deploy SOP A (dup path lower score)",
          path: "vault/20_notes/deploy-a.md",
          status: "active",
          source_type: "manual",
          snippet: "重复路径的另一块，不应单独成卡。",
          chunk_id: "chunk:a2",
          score: 3,
        },
        {
          title: "Deploy SOP B",
          path: "vault/20_notes/deploy-b.md",
          status: "active",
          source_type: "manual",
          snippet: "备选部署笔记：使用 docker compose up -d。",
          chunk_id: "chunk:b1",
          heading_path: ["备选"],
          score: 8,
        },
    ...
    const context = await getQueryAgentContext("如何部署 Personal Wiki", undefined, {
      budgetTokens: 1800,
      topK: 8,
    });

    expect(context.evidence.cards.length).toBeGreaterThanOrEqual(1);
    expect(context.evidence.cards.length).toBeLessThanOrEqual(8);
    const paths = context.evidence.cards.map((card) => card.path);
    expect(paths).toEqual(Array.from(new Set(paths)));
    expect(context.evidence.cards[0]).toMatchObject({
      path: "vault/20_notes/deploy-a.md",
      kind: expect.stringMatching(/^(chunk|section|document|note)$/),
      headingPath: ["部署", "安装"],
      chunkId: "chunk:a1",
      expand: expect.objectContaining({
        neighbor: expect.stringContaining("neighbor"),
        section: expect.stringContaining("section"),
      }),
    });
    expect(context.evidence.cards[0].text).toContain("docker compose");
    const wikiMemory = context.memoryItems.find(
      (item) =>
        item.type === "wiki" &&
        item.metadata.path === "vault/20_notes/deploy-a.md",
    );
    expect(wikiMemory?.metadata).toMatchObject({
      path: "vault/20_notes/deploy-a.md",
      chunkId: "chunk:a1",
  ...
  it("expands how-to deploy queries to section without expanding bare SOP labels", async () => {
    mockedSearchWikiChunks.mockResolvedValue({
      status: "ok",
      retrieval: "fts5-bm25-structured-v2",
      results: [
        {
          ...
          path: "vault/20_notes/wiki-deploy.md",
          status: "active",
          source_type: "manual",
          snippet: "短片段",
          chunk_id: "chunk:wiki-deploy",
        },
      ],
    });
    mockedExpandWikiChunk.mockResolvedValue({
      status: "ok",
      level: "section",
      content: "完整部署步骤：1. 拉代码 2. docker compose up 3. 校验 health",
      range_basis: "indexed-snapshot",
      estimated_tokens: 40,
      available_estimated_tokens: 40,
      truncated: false,
    });

    const expanded = await searchWikiContext(["如何部署 Personal Wiki"], 4, {}, {
      expansionQuery: "如何部署 Personal Wiki",
    });
    expect(mockedExpandWikiChunk).toHaveBeenCalledWith(
      "chunk:wiki-deploy",
      "section",
      expect.objectContaining({
        ...
        query: "如何部署 Personal Wiki",
      }),
    );
    expect(expanded.candidates[0].excerpt).toContain("docker compose");

    mockedExpandWikiChunk.mockClear();
    await searchWikiContext(["Personal OS SOP"], 4, {}, {
      expansionQuery: "Personal OS SOP",
    });
    expect(mockedExpandWikiChunk).not.toHaveBeenCalled();
  });

  it("keeps Wiki body text in memoryItems only in the serialized context pack", async () => {
    const uniqueBody = "ExactBudgetMarker UNIQUE_WIKI_BODY_7A91 " + "证据".repeat(120);
    mockedSearchWikiChunks.mockResolvedValue({
      status: "ok",
      retrieval: "fts5-bm25-structured-v2",
      results: [
        {
          title: "ExactBudgetMarker",
          path: "vault/20_notes/exact-budget.md",
          status: "active",
          source_type: "manual",
          snippet: "短片段",
          chunk_id: "chunk:exact-budget",
        },
      ],
    });
    mockedExpandWikiChunk.mockResolvedValue({
      ...
      available_estimated_tokens: 150,
      truncated: false,
    });

    const context = await getQueryAgentContext(
      "请返回全文 ExactBudgetMarker",
      undefined,
      { budgetTokens: 200 },
    );

    const wikiMemory = context.memoryItems.find((item) => item.type === "wiki");
    expect(wikiMemory?.summary).toContain("UNIQUE_WIKI_BODY_7A91");
    expect(context.wiki.candidates[0]).not.toHaveProperty("excerpt");
    expect(context.evidence.episodes[0]).toMatchObject({
      type: "wiki",
      summary: "",
      memoryId: "wiki:vault/20_notes/exact-budget.md",
    });
    // Structured cards are the EvidencePack body; memoryItems keep the same
    // summary for budget/tiers. wiki.candidates metadata view must stay empty.
    expect(
      context.evidence.cards.some((card) =>
        card.text.includes("UNIQUE_WIKI_BODY_7A91"),
      ),
    ).toBe(true);
    expect(JSON.stringify(context).match(/UNIQUE_WIKI_BODY_7A91/g)?.length).toBe(
      2,
    );
    expect(context.tokenBudget.estimatedTokens).toBeLessThanOrEqual(160);
  });

  it("preserves a query anchor beyond 700 characters in an expanded Wiki memory", async () => {
    mockedSearchWikiChunks.mockResolvedValue({
      status: "ok",
      retrieval: "fts5-bm25-structured-v2",
      results: [
        {
          title: "TailAnchorMarker",
          path: "vault/20_notes/tail-anchor.md",
          status: "active",
          source_type: "manual",
          snippet: "短片段",
          chunk_id: "chunk:tail-anchor",
        },
      ],
    });
    mockedExpandWikiChunk.mockResolvedValue({
      ...
      content:
        ...
        "filler ".repeat(220) +
        "TailAnchorMarker decisive evidence",
      range_basis: "indexed-snapshot",
      estimated_tokens: 420,
      available_estimated_tokens: 420,
      truncated: false,
    });

    const context = await getQueryAgentContext(
      "请返回全文 TailAnchorMarker",
      undefined,
      { budgetTokens: 500 },
    );

    const wikiMemory = context.memoryItems.find((item) => item.type === "wiki");
    expect(wikiMemory?.summary).toContain("TailAnchorMarker decisive evidence");
    expect(wikiMemory?.summary.length).toBeGreaterThan(700);
    expect(wikiMemory?.summary).not.toContain("[unrelated]");
  });

  it("keeps automatic full-document expansion inside the total context budget", async () => {
    mockedSearchWikiChunks.mockResolvedValue({
      status: "ok",
      retrieval: "fts5-bm25-structured-v2",
      results: [
    ...
    mockedExpandWikiChunk.mockResolvedValue({
      status: "ok",
      level: "document",
      content: "证据".repeat(1_000),
      range_basis: "indexed-snapshot",
      estimated_tokens: 2_000,
      available_estimated_tokens: 2_000,
      truncated: false,
    });

    const result = await searchWikiContext(
      ["请返回全文 Deployment evidence"],
      8,
      {},
      {
        budgetTokens: 200,
        expansionQuery: "请返回全文 Deployment evidence",
      },
    );
    ...
    const expansionOptions = mockedExpandWikiChunk.mock.calls[0][2] as {
      maxTokens: number;
    };
    expect(expansionOptions.maxTokens).toBeLessThanOrEqual(160);
    expect(result.candidates[0].excerpt?.length).toBeLessThan(2_000);
    expect(result.candidates[0].metadata).toMatchObject({
      context_budget_truncated: true,
      context_estimated_tokens: expect.any(Number),
    });
    expect(Number(result.candidates[0].metadata?.context_estimated_tokens)).toBeLessThanOrEqual(
      160,
    );
  });
  ...
  it("returns the same context envelope for keyword-only lookups", async () => {
    ...
    // Pure knowledge query: do not inject sticky global task guidance.
    expect(context.nextAction).toBe("");
  });
  ...
  it("does not inject failed AgentRun guidance for pure knowledge queries", async () => {
    mockedSearchWikiNotes.mockResolvedValue([]);
    const taskFindMany = vi.fn().mockResolvedValue([]);
    const activityLogFindMany = vi.fn().mockResolvedValue([
      {
  ...
  it("applies requested budget and exposes cited hot items plus omissions", async () => {
    ...
    expect(context.tokenBudget.estimatedTokens).toBeLessThanOrEqual(
      context.tokenBudget.maxTokens - context.tokenBudget.reservedTokens,
    );
    expect(context.tiers.hot.items[0]).toMatchObject({
      type: "task",
      id: "task_hot_budget",
  ...
  it("adds per-tier token budgets and archives traceable tier overflow", async () => {
    ...
    expect(
      context.wiki.candidates.reduce(
        (total, candidate) =>
          total + Number(candidate.metadata?.context_estimated_tokens ?? 0),
        0,
      ),
    ).toBeLessThanOrEqual(192);
    // High-score wiki may land in hot (aligned with memory tier thresholds);
    // overflow archive can therefore appear in hot or warm.
    const archivedWiki = [
      ...context.tiers.hot.archived,
      ...context.tiers.warm.archived,
    ].filter((item) => item.type === "wiki");
    expect(archivedWiki.length).toBeGreaterThan(0);
    expect(archivedWiki[0]).toMatchObject({
