 M src/app/api/agent/context/route.ts
 M src/lib/agent-context.ts
 M src/lib/validation.ts
 M src/lib/wiki-ingest.ts
 M tests/routes/intake-wiki-fallback.test.ts
 M tests/services/agent-context.test.ts
 M tests/services/tasks.test.ts
 M tests/services/wiki-ingest.test.ts
?? docs/CLASSIC_KNOWLEDGE_OBJECT_MANIFEST.md
?? docs/sources/
?? examples/
?? schemas/
?? scripts/archive-agent-run-context-pack.mjs
?? scripts/lint-classic-knowledge-object-manifest.mjs
?? tests/services/agent-run-context-pack.test.ts
?? tests/services/knowledge-manifest.test.ts
